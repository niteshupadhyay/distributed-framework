...
void SSSP(char *edge, char *part) {
	...
	clEnqueueReadBuffer(graph.command_queue,graph.properties[3],CL_TRUE,0,sizeof(int )*graph.property_size,(void *)(((struct struct_hgraph *)hgraph.extra)->old_dist),0,NULL,NULL);
	bool  falcvt1;
	falcvt1=true;
		...
		for(int i=0;i<graph.no_of_nodes;i++){
			if(i==rank){
				for(int j=0;j<graph.no_of_nodes;j++){	
					if(j!=i){
						int *id_buf = (int *)malloc(sizeof(int)*hgraph.no_of_outgoing_vertices[j]);
						int  *val_buf = (int  *)malloc(sizeof(int )*hgraph.no_of_outgoing_vertices[j]);
						int actual_data_send = 0;
						int  *ptr = (((struct struct_hgraph *)hgraph.extra)->dist)+hgraph.offset[j];
						int  *old_ptr = (((struct struct_hgraph *)hgraph.extra)->old_dist)+hgraph.offset[j];
						for(int k=0;k<hgraph.no_of_outgoing_vertices[j];k++){
							if(old_ptr[k] > ptr[k]){
								id_buf[actual_data_send] = hgraph.outgoing_vertices[j][k];
								val_buf[actual_data_send] = ptr[k];
								actual_data_send++;
								old_ptr[k] = ptr[k];
							}
						}
						MPI_Send(&actual_data_send,1,MPI_INT,j,0,MPI_COMM_WORLD);
						MPI_Send(id_buf,actual_data_send,MPI_INT,j,0,MPI_COMM_WORLD);
						MPI_Send(val_buf,actual_data_send,MPI_INT ,j,0,MPI_COMM_WORLD);
					}
				}
			}
			else{
				int actual_data_rcv = 0;
				MPI_Recv(&actual_data_rcv,1,MPI_INT,i,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
				int *id = (int *)malloc(sizeof(int)*actual_data_rcv);
				int  *rcv = (int  *)malloc(sizeof(int )*actual_data_rcv);
				MPI_Recv(id,actual_data_rcv,MPI_INT,i,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
				MPI_Recv(rcv,actual_data_rcv,MPI_INT ,i,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
				for(int k=0;k<actual_data_rcv;k++){
					int index = hgraph.partition_indices[id[k]]+hgraph.offset[rank];
...
int main(int argc,char*argv[]) {
	...
	is_all_selective = false;
	FILE *fp;
	char fileName[] = "sssp2.cl";
	fp = fopen(fileName, "r");
